...
/opt/spark/bin/spark-submit     --master k8s://https://10.1.2.61:6443     --deploy-mode cluster     --name hello-0    \
 ...
 --conf spark.sparkmeasure.prometheusStagemetrics=true     --conf "spark.master=k8s://https://10.1.2.61:6443"    \
 ...
 --conf "spark.kubernetes.scheduler.volcano.podGroupTemplateFile=pg-hello-0.yaml"     --conf "spark.kubernetes.driver.master= https://10.1.2.61:6443"     --conf "spark.kubernetes.driver.pod.featureSteps=org.apache.spark.deploy.k8s.features.VolcanoFeatureStep"      --conf "spark.kubernetes.driver.label.app=hello-0"     --conf "spark.kubernetes.driver.request.cores=1"     --conf "spark.kubernetes.driver.limit.cores=1"     \
 --conf "spark.kubernetes.driver.secrets.apiserver=/opt/pki"     --conf "spark.kubernetes.executor.deleteOnTermination=true"     --conf "spark.kubernetes.executor.request.cores=1"     --conf "spark.kubernetes.executor.limit.cores=1"     --conf "spark.kubernetes.executor.podNamePrefix=hello-0"      --conf "spark.kubernetes.executor.scheduler.name=volcano"     --conf "spark.kubernetes.executor.pod.featureSteps=org.apache.spark.deploy.k8s.features.VolcanoFeatureStep"     --conf "spark.kubernetes.authenticate.driver.mounted.oauthTokenFile=/opt/pki/token"     --conf "spark.kubernetes.authenticate.driver.caCertFile=/opt/pki/ca.crt"      --conf "spark.kubernetes.authenticate.driver.serviceAccountName=my-release-spark"     --conf "spark.kubernetes.authenticate.driver.oauthTokenFile=/opt/pki/token"     --conf "spark.kubernetes.authenticate.executor.serviceAccountName= my-release-spark"     --conf "spark.kubernetes.authenticate.caCertFile=/opt/pki/ca.crt"     --conf "spark.kubernetes.authenticate.oauthTokenFile=/opt/pki/token"     --conf "spark.dynamicAllocation.executorIdleTimeout=10s"     --conf "spark.dynamicAllocation.cachedExecutorIdleTimeout=200s"     --conf "spark.dynamicAllocation.minExecutors=1"     --conf "spark.dynamicAllocation.initialExecutors=1"     --conf "spark.dynamicAllocation.maxExecutors=2"      --conf "spark.dynamicAllocation.executorAllocationRatio=0.5"     --conf "spark.dynamicAllocation.enabled=true"     \
 --conf "spark.dynamicAllocation.shuffleTracking.enabled=true"        --conf "spark.metrics.conf.*.source.jvm.class"="org.apache.spark.metrics.source.JvmSource"     \
